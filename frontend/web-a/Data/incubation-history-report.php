...
$result=$conn->query("SELECT * FROM inclubation_history order by stock_history_id DESC");
...
while ($row=mysqli_fetch_array($result)) {
	...
	while ($patient=mysqli_fetch_array($patient_name)) {
			$outp .='{"patient_name":"' .$patient["patient_name"] .'",';
				...
				//SELECT item_name related to id
				$inventory_item_id=$conn->query("SELECT inventory_item_id from incubation_stock
					where stock_id = ".$row["incubation_stock_stock_id"]);
				// while ($item=mysqli_fetch_array($item_name)) {
				//
				// }
